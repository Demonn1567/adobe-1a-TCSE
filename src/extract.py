...
def _ocr_page(pix: fitz.Pixmap, langs="eng+jpn+hin") -> List[Span]:
    ...
    return [Span(text, 0, (0,0,pix.width,pix.height), 0, "OCR", False, False, _guess_lang(text))]
...
def _merge_line_spans(raw: List[Span]) -> List[Span]:
    ...
    for nxt in raw[1:]:
        if nxt.page != buf.page:
            merged.append(buf); buf = nxt; continue

        # special: handle "2." + "Introduction …" split by badge box
        if SECTION_NUM_RE.fullmatch(buf.text.strip()) and not sec_prefix(nxt):
            ...
            if diff_sec:
                ...
                merge = (same_baseline and gap_ok) or (same_font and vert_ok) or buf.text.rstrip().endswith(("–", "."))

        if merge:
            buf.text = f"{buf.text} {nxt.text}"
            buf.bbox = (buf.bbox[0], buf.bbox[1], nxt.bbox[2], max(buf.bbox[3], nxt.bbox[3]))
        else:
            merged.append(buf); buf = nxt
    merged.append(buf)
    return merged

# ---------- public --------------------------------------------------------- #
def extract_spans(pdf_path: pathlib.Path, dpi=150) -> List[Span]:
    doc = fitz.open(pdf_path); spans: List[Span] = []
    for i in range(doc.page_count):
        page = doc.load_page(i); d = page.get_text("dict")
        if not any(b["type"] == 0 for b in d["blocks"]):   # scanned
            for sp in _ocr_page(page.get_pixmap(dpi=dpi)): sp.page = i+1; spans.append(sp); continue
        ...
        for b in d["blocks"]:
            ...
            for l in b["lines"]:
                for s in l["spans"]:
                    txt = s["text"].strip()
                    if txt:
                        raw.append(Span(txt, i+1, tuple(s["bbox"]), s["size"], s["font"],
                                        bool(s["flags"]&2), bool(s["flags"]&1), _guess_lang(txt)))
        raw.sort(key=lambda s:(s.page, s.bbox[1], s.bbox[0]))
        spans.extend(_merge_line_spans(raw))
    doc.close(); return spans
